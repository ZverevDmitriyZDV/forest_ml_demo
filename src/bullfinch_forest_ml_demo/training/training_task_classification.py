...
def _select_features(df: pd.DataFrame, cfg: ClassificationConfig) -> Tuple[pd.DataFrame, pd.Series]:
    # Drop IDs + columns that can leak / not needed
    ...
    # Optional: drop labels that are derived targets we don't want as features
    # (estimated_age/biomass/risk_flag are targets for other tasks)
...
def run_task_classification(cfg: ClassificationConfig) -> Dict[str, object]:
    ...
    return {
        ...
        "pipeline": pipe,  # for saving later
